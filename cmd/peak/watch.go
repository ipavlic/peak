...
func runWatch(dir string) error {
	...
	return watchLoop(ctx, watcher, dir)
}
...
func watchLoop(ctx context.Context, watcher *fsnotify.Watcher, dir string) error {
	...
	for {
		...
		case event, ok := <-watcher.Events:
			...
			debounceTimer = handleFileEvent(ctx, event, dir, debounceTimer)
...
func handleFileEvent(ctx context.Context, event fsnotify.Event, dir string, debounceTimer *time.Timer) *time.Timer {
